...
def main(cfg):
    ...
    # ------------------------------------------------------------------
    log_T = []
    student_ckpts = sorted(out_root.glob("checkpoint-T-*.pt"),                           key=lambda p: int(p.stem.split('-')[-1]))
    if not student_ckpts:
        raise RuntimeError("checkpoint-T-*.pt 파일을 찾을 수 없습니다!")

    for p in student_ckpts:
    ...
    # ──────────────────────────────────────────────────────────────
    # ❶ Teacher plateau 시작(idx 3) 하나만 사용해 Phase-A
    # ──────────────────────────────────────────────────────────────
    step_sel = ckpts[2]                       # 0-base ⇒ 세 번째 = idx 3
    ...
    log_A   = train(small_S, tr_S, ev_S, kd=True, teacher=teacher, lr=cfg.lr,
                    alpha=cfg.alpha, temp=cfg.temp, tag="A", out_dir=cfg.out,
                    eval_steps=cfg.eval_steps, save_steps=cfg.save_steps,
                    max_steps=phase_steps)

    # ──────────────────────────────────────────────────────────────
    # ❷ Phase-A 로그에서 4 ckpt 선택  → (B,C) 루프
    # ──────────────────────────────────────────────────────────────
    ckpts_A = pick_ckpts(acc_pairs(log_A))

    for jdx, step_A in enumerate(ckpts_A, 1):
        # --- Small-teacher 준비 (A ckpt)
        small_T = build_mlp(cfg.data_dim, cfg.n_labels, cfg.student_h)
        small_T.load_state_dict(torch.load(out_root / f"checkpoint-A-{step_A}.pt",
                                           map_location=DEVICE))

        # Phase-B : Small-T → Big-S
        big_S = build_mlp(cfg.data_dim, cfg.n_labels, cfg.teacher_h)
        train(big_S, tr_S, ev_S, kd=True, teacher=small_T, lr=cfg.lr,
              alpha=cfg.alpha, temp=cfg.temp, tag=f"B{jdx}", out_dir=cfg.out,
              eval_steps=cfg.eval_steps, save_steps=cfg.save_steps,
              max_steps=phase_steps)

        # --- Big-teacher 준비 (B 최종 ckpt)
        big_T = build_mlp(cfg.data_dim, cfg.n_labels, cfg.teacher_h)
        big_T.load_state_dict(torch.load(out_root / f"checkpoint-B{jdx}-{phase_steps}.pt",
                                         map_location=DEVICE))

        # Phase-C : Big-T → Fresh Small-S
        fresh_S = build_mlp(cfg.data_dim, cfg.n_labels, cfg.student_h)
        train(fresh_S, tr_S, ev_S, kd=True, teacher=big_T, lr=cfg.lr,
              alpha=cfg.alpha, temp=cfg.temp, tag=f"C{jdx}", out_dir=cfg.out,
              eval_steps=cfg.eval_steps, save_steps=cfg.save_steps,
              max_steps=phase_steps)

    print("[DONE] Phase-A 1 회 + (B,C)×4 회 완료")
